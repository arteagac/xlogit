...
if(dataset == "artificial"){
    ...
    run_estimation = function(){
        gmnl(choice~price+time+conven+comfort+meals+petfr+emipp+nonsig1+nonsig2+nonsig3|0, Artif,
            model = "mixl", ranp=c(meals="n", petfr="n", emipp="n"), 
...
if(dataset == "electricity"){
    ...
    run_estimation = function(){
        model = gmnl(choice~pf+cl+loc+wk+tod+seas|0, Electr,
                model = "mixl", ranp=c(pf="n",cl="n",loc="n",wk="n",tod="n",seas="n"), 
...
if(profile){
    ...
    cat(paste("gmnl", dataset, n_draws, time, logLik(model)[1], mem, 0,
    ...
    varnames = names(summ)
    est_coeff = as.vector(summ)
    est_stder =  as.vector(sqrt(diag(vcov(model))))
